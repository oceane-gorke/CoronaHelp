...
def get_donne_identification(request):
    if request.method == 'POST': #le formulaire a été posté
        ...
        if form.is_valid():
            ...
            if ModelHabitant.objects.get(email=form.cleaned_data['email']) is not None and ModelHabitant.objects.get(mdp=form.cleaned_data['mdp']) is not None:
...
def get_donne_identification_collectivite(request):
    if request.method == 'POST': #le formulaire a été posté
        ...
        if form.is_valid():
            print('ville:', form.cleaned_data['ville']) #on affiche la donnée saisie dans le formulaire
            print('mdp:', form.cleaned_data['mdp'])
            if ModelCollectivite.objects.get(ville=form.cleaned_data['ville']) is not None and ModelCollectivite.objects.get(mdp=form.cleaned_data['mdp']) is not None:
                print('Vérification en cours...')
                #return HttpResponseRedirect(reverse('thanks3'))
                return render(request, 'appone/gestioncollectivite.html')
            else:
                return HttpResponse('Identification échoué. Veuillez réessayer...')
    else: #on fait un GET, on veut juste afficher le formulaire dans le navigateur
...
def cartefoyer(request):
    ...
    x=[]
    y=[]
    with open('/Users/gorkeoceane/CoronaHelp2.0/appone/dataset.csv', 'r') as csvfile:
...
def cartefrequence(request):
    ...
    x2=[]
    y2=[]
    with open('/Users/gorkeoceane/CoronaHelp2.0/appone/dataset2.csv', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter=',')
        for row in plots:
            x2.append(int(row[0]))
            y2.append(int(row[1]))
...
def produits(request):
    ...
    fig3 = plt.figure()
    y3=[]
    with open('/Users/gorkeoceane/CoronaHelp2.0/appone/dataset3.csv', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter=',')
        for row in plots:
            y3.append(int(row[0]))
    x3=np.arange(6)
...
def regimes(request):
    fig4 = Figure()
    y4=[]
    with open('/Users/gorkeoceane/CoronaHelp2.0/appone/dataset4.csv', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter=',')
        for row in plots:
            y4.append(int(row[0]))
    x4 = np.arange(6)
